...
exports.createBill = async (req, res) => {
    try{
        if(!req.body.date || !req.body.email || !req.body.company || !req.body.bl_no){
            return (
                res.json({
        ...
        const found = await billModel.findOne({vatNo : req.body.vatNo , company : {$ne : req.body.company}});
        if(found){
            return(
                res.json({
                    message : "This vatNO is already assigned to some other company , You cannot use one vatNO for more than one company",
                    status : false
                })
            )
        }
...
exports.getAllBillsByCompanyName = async (req, res) => {
    try{
        ...
        const result = await billModel.find({company : {$regex : companyName , $options : "i" }}).skip((parseInt(page)-1 )*parseInt(limit)).limit(parseInt(limit)).sort({$natural : -1});
...
exports.getAllBillsByVatNo = async (req, res) => {
    try{
        ...
        const result = await billModel.find({vatNo : vatNo}).skip((parseInt(page)-1 )*parseInt(limit)).limit(parseInt(limit)).sort({$natural : -1});
...
exports.updateBillPaidAmount= async (req, res) => {
    try{
        ...
        const result = await billModel.findOneAndUpdate({_id : bill_id} , {
