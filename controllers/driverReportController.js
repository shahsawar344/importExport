...
exports.getDriverReportsNyName= async (req, res) => {
    try{
        ...
        const result = await driverReportModel.find({name : name}).skip((parseInt(page)-1 )*parseInt(limit)).limit(parseInt(limit)).sort({$natural : -1});
...
exports.getAllDriverReports= async (req, res) => {
    try{
        ...
        const result = await driverReportModel.find().skip((parseInt(page)-1 )*parseInt(limit)).limit(parseInt(limit)).sort({$natural : -1});
        if(!result){
            return(
                res.json({
                    message : "Driver reports could not Fetched",
